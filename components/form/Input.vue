	...
	<ValidationProvider tag="div" :vid="name" :name="name" :rules="inputRules" mode="passive" v-slot="v" :class="componentClass">
		...
		<label v-else-if="type === 'select'" :for="name">
			...
			<select
				...
				@input="updateValue($event.target.value)"
			>
				<option v-for="(optionLabel, key) in options" :key="key" :value="key">{{ optionLabel }}</option>
			</select>
		</label>
		<label v-else :for="name">
			...
			<span v-if="type === 'password' && !suffix" class="input__suffix input__suffix--hover">
				...
				<span v-else class="fas fa-eye-slash" @click="passwordVisible = false"></span>
			</span>
		</label>
	</ValidationProvider>
</template>
...
.input__label {
	...
	transition: transform .2s, font-size .3s;

	.input--focus &, .input:not(.input--empty) & {
